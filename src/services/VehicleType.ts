...
import { VehicleType } from "@/interfaces/Models";
...
export default class VehicleTypeService {
	...
	/**
	 * Requests all the vehicle types.
	 */
	async all() : Promise<VehicleType[]> {
		try {
			return await apiRequest().get('/vehicle-types').then((res) => res.data);
		} catch(catchError) {
