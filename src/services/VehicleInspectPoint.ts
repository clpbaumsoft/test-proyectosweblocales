...
export default class VehicleInspectPointService {

	/**
	 * Requests all the vehicle inspect points.
	 */
	async all() : Promise<VehicleInspectPoint[]> {
		try {
			return await apiRequest().get('/vehicle-inspect-points').then((res) => res.data);
		} catch(catchError) {
