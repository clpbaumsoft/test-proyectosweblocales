...
export default class EntryService {
	...
	/**
	 * Add a entry for the visitor
	 * @returns 
	 */
	async give(visitId: number, visitorId: number, data: CreateEntryFormType) {
		try {
			return await apiRequest().post(`/visits/${visitId}/visitors/${visitorId}/give-entry`, data).then((res) => res.data);
		} catch(catchError) {
	...
	/**
	 * Leave the active entry for the visitor
	 * @returns 
	 */
	async leave(visitorId: number) {
		try {
			return await apiRequest().post(`/visitors/${visitorId}/leave-entry`).then((res) => res.data);
		} catch(catchError) {
			const error = catchError as AxiosError
			const status = error?.status || 500
			const dataResponse = (error?.response?.data as ErrorResponseDataType) || { error: "" }
