...
import { VisitorType } from "@/interfaces/Models";
import { ErrorResponseDataType } from "@/interfaces/General";

//Libs
import apiRequest from "@/lib/ApiRequest";

export default class VisitorTypeService {

	/**
	 * Requests all the visitor types.
	 */
	async all() : Promise<VisitorType[]> {
		try {
			return await apiRequest().get('/visitor-types').then((res) => res.data);
		} catch(catchError) {
