...
import { CareCompany } from "@/interfaces/Models";
import { ErrorResponseDataType } from "@/interfaces/General";

//Libs
import apiRequest from "@/lib/ApiRequest";

export default class CareCompanyService {

	/**
	 * Requests all the care companies.
	 */
	async all() : Promise<CareCompany[]> {
		try {
			return await apiRequest().get('/carecompanies').then((res) => res.data);
		} catch(catchError) {
