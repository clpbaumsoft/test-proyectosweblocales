...
export default function useVisitsDashboardByApproverDocsUser() {
	...
	const loadVisits = useCallback(async (currentPage: number, currentRowsPerPage: number) => {
		...
		debounce(async () => {
			try {
				const responsePaginateVisits = await Orchestra.visitService.allVisitbyApproverDocsUser(currentPage, currentRowsPerPage, filter)
	...
	const loadAllVisitsForFilter = useCallback(async () => {
		...
		try {
			...
			const responsePaginateVisits = await Orchestra.visitService.allVisitbyApproverDocsUser(0, 99999, filter)
	...
	const loadVisitsFirstTime = async (filter: string = 'activated') => {
		...
		await debounce(async () => {
			try {
				const responsePaginateVisits = await Orchestra.visitService.allVisitbyApproverDocsUser(page, rowsPerPage, filter)
				setTotal(responsePaginateVisits.total)
